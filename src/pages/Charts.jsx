...
export default function Charts() {
  ...
  const activeColor = selectedMetal === 'gold' ? '#D4AF37' : '#94A3B8';
  const activeBgClass = selectedMetal === 'gold' ? 'bg-[#D4AF37]' : 'bg-slate-500';
  ...
  const getPoints = () => {
    ...
    return currentData.map((item, index) => {
      const x = (index / (currentData.length - 1)) * (width - 10) + 5;
      const y = height - ((item.price - minPrice) / range) * 65 - 20;
      return { x, y, ...item };
    });
  };
    ...
    <div className="space-y-4 max-w-2xl mx-auto p-2 pb-6">
      ...
      <div className="flex bg-muted p-1 rounded-xl max-w-xs mx-auto">
        ...
        <button
          ...
          className={`flex-1 py-1.5 text-xs font-semibold rounded-lg capitalize transition ${
            selectedMetal === 'silver' ? 'bg-slate-600 text-white shadow-sm' : 'text-muted-foreground'
          }`}
        >
          Silver
        </button>
        ...
        {['1D', '1W', '1M', '1Y', '5Y', '10Y'].map((tf) => (
          <button
            ...
            className={`px-3 py-1 rounded-md text-xs font-semibold transition ${
              timeframe === tf
                ? `${activeBgClass} text-white shadow-sm`
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-4 relative">
        <div className="min-h-[36px] flex items-center justify-between border-b border-border/50 pb-2">
          {hoveredPoint ? (
            <div className="flex justify-between items-center w-full bg-muted/60 px-3 py-1.5 rounded-lg border border-border transition-all">
              <span className="text-xs font-semibold text-muted-foreground">
                Period: <strong className="text-foreground">{hoveredPoint.label}</strong>
              </span>
            ...
            {currentData.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  onTouchStart={() => setHoveredPoint(item)}
                ...
                >
                  ...
                  <div
                    ...
                    className={`w-full rounded-t-md transition-all duration-300 ${
                      isHovered ? 'shadow-lg scale-105 opacity-100' : 'opacity-80 group-hover:opacity-100'
                    }`}
                  />
              ...
              {currentData.map((item, idx) => (
                <div key={idx} className="text-center">
                  ...
                  <p
                    ...
                    style={{
                      color: hoveredPoint?.label === item.label ? activeColor : 'inherit'
                    }}
                  >
                    {formatShortValue(item.price)}
                  </p>
