...
export default function History() {
  ...
  const getDynamicHistoricalRate = (selectedDateStr) => {
    // Timezone offset correction to prevent UTC date rollback
    ...
    const usdVariance = parseFloat((Math.sin(day * 7) * 1.5).toFixed(2));

    let baseUsdPkr = 278.70;
    let baseGoldPKR = 454300;

    if (year >= 2026) {
      baseUsdPkr = 278.70 + usdVariance;
      baseGoldPKR = 450000 + (month * 500);
    } else if (year === 2025) {
      baseUsdPkr = 281.50 + usdVariance;
      if (month === 12) {
      ...
      baseUsdPkr = 278.50 + usdVariance;
      baseGoldPKR = 250000 + (month * 3000);
    } else if (year === 2023) {
      baseUsdPkr = 282.00 + usdVariance;
      baseGoldPKR = 200000 + (month * 1800);
    } else if (year === 2022) {
      baseUsdPkr = 204.00 + usdVariance;
      baseGoldPKR = 145000 + (month * 500);
    } else if (year === 2021) {
      baseUsdPkr = 170.00 + usdVariance;
      baseGoldPKR = 125000 + (month * 500);
    } else if (year === 2020) {
      baseUsdPkr = 160.20 + usdVariance;
      baseGoldPKR = 110000 + (month * 400);
    } else {
      baseUsdPkr = 120.00 + usdVariance;
  ...
  const safeFormatCurrency = (val, cur, usdRate) => {
    if (!val || isNaN(val)) return 'N/A';
    try {
      return formatCurrency(val, cur, usdRate);
    } catch {
      return `Rs ${val.toLocaleString('en-PK')}`;
    }
  };

  const safeGetUsdSubtext = (val, cur, usdRate) => {
    if (!val || isNaN(val)) return null;
    try {
      return getUsdSubtext(val, cur, usdRate);
    } catch {
      return null;
    }
  };
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="rounded-2xl border border-[#D4AF37]/30 bg-gradient-to-br from-[#D4AF37]/10 to-transparent p-4">
            ...
            <p className="text-2xl font-bold mt-1 text-foreground">
              {safeFormatCurrency(result.gold_per_tola_pkr, currency, result.usd_pkr)}
            </p>
            {safeGetUsdSubtext(result.gold_per_tola_pkr, currency, result.usd_pkr) && (
          ...
          <div className="rounded-2xl border border-[#C0C0C0]/30 bg-gradient-to-br from-[#C0C0C0]/10 to-transparent p-4">
            ...
            <p className="text-2xl font-bold mt-1 text-foreground">
              {safeFormatCurrency(result.silver_per_tola_pkr, currency, result.usd_pkr)}
            </p>
            {safeGetUsdSubtext(result.silver_per_tola_pkr, currency, result.usd_pkr) && (
              <p className="text-xs text-muted-foreground mt-0.5">
                {safeGetUsdSubtext(result.silver_per_tola_pkr, currency, result.usd_pkr)}
              </p>
            )}
          ...
          <div className="rounded-2xl border border-[#008B8B]/30 bg-gradient-to-br from-[#008B8B]/10 to-transparent p-4">
            ...
            <p className="text-2xl font-bold mt-1 text-foreground">
              {safeFormatCurrency(result.platinum_per_tola_pkr, currency, result.usd_pkr)}
            </p>
            {safeGetUsdSubtext(result.platinum_per_tola_pkr, currency, result.usd_pkr) && (
              <p className="text-xs text-muted-foreground mt-0.5">
                {safeGetUsdSubtext(result.platinum_per_tola_pkr, currency, result.usd_pkr)}
              </p>
            )}
          ...
          <div className="rounded-2xl border border-[#D2691E]/30 bg-gradient-to-br from-[#D2691E]/10 to-transparent p-4">
            ...
            <p className="text-2xl font-bold mt-1 text-foreground">
              {safeFormatCurrency(result.copper_per_tola_pkr, currency, result.usd_pkr)}
            </p>
            {safeGetUsdSubtext(result.copper_per_tola_pkr, currency, result.usd_pkr) && (
              <p className="text-xs text-muted-foreground mt-0.5">
                {safeGetUsdSubtext(result.copper_per_tola_pkr, currency, result.usd_pkr)}
              </p>
            )}
